...
class MobileBanking(BankAccount):
    ...
    def __init__(self, name, money, phone):
            ...
            self.phone = phone
    #ให้ทำการเช็คก่อนว่า user มีเงินพอต่อการถอนหรึ่อไม่        
    
    def __sub__(self, other):
        money_save=self.money - other.money
        if money_save >= 0:
...
if __name__ == "__main__":
    ...
    print(s2)
    s3 =MobileBanking("Aris",1500, "0643197367")
    print(s3)
    s4 =MobileBanking("mark",500,"0643197367")
    s5 =MobileBanking("mark",1500,"0643197367")
    moneyme= s4-s5
    print(moneyme)
    print("-"*100)
  #สร้าง instance 
    print("...สร้าง instance ...") 
    newusers = []
    for ojt in users:
        n = list(ojt.values())
        newusers.append(n)
        #เรียงตามตัวอักษรของชื่อ user
        newusers.sort(key=sortbysec)

        resuit=sorted(newusers)
    print("...ทำการเรียงตามจำนวนเงิน...") 
    print(newusers)
    print("-"*100)
    print("...ทำการเรียงตามตัวอักษร...") 
    print(resuit)

    print("-"*100)
    print("...เปิดบัญชีธนาคาร...")

    for user in resuit:
        data=BankAccount(user[0],user[1])
        print(data)
    print("-"*100)
    print("...เปิดบัญชี MobileBanking ...")
    print("...ต้องมีเบอร์โทรและเงินขั้นต่ำ500 ...")

    #แสดงข้อมูลของ user ใน format '<name>: <money>'และต้องมีเงืนอย่างน้อย 500 บาท และมีเบอร์โทรศัพท์
    for user in resuit:
